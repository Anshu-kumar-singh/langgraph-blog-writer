from __future__ import annotations # Allows Python to treat type hints as strings internally.

import operator # used later for combining state values (operator.add).
...
# This schema stores one research result/source collected from the web.
...
# This schema stores the details for one image that will be generated and inserted into the blog.
# the image will be generated by llm only it will store the details .
...
llm_70b = ChatGroq(model="openai/gpt-oss-120b")

# -----------------------------
# 3) Router

# prompt instruction given to the router llm to decide whether research is needed.
...
# it checks that research is needed or not .
...
# This function performs the actual web search using Tavily.
def _tavily_search(query: str, max_results: int = 5) -> List[dict]:
    ...
    try:
        ...
        return []
# This is a small helper function that converts a string date into a Python date object safely.
...
# 6) Fanout
# -----------------------------
# Its job is to create parallel worker jobs.
...
# This node runs after the entire blog is written.
...
# This is the beginning of the node that actually generates the images and inserts them into the blog.
